#!/usr/bin/env python3
"""A6-FIT-v1.2 causal Transformer fixed64 scratch fit to 6k."""
...
from a6_operation_models import ACTION_DIM, ACTION_HORIZON, HIDDEN_DIM, OperationCausalAbsolute
from path_config import JOINTTRAIN_ARCH6_O030R_RESULT_ROOT, JOINTTRAIN_ARCH6_O030S_RESULT_ROOT
...
RUN_ID = "a6_o030s_causal_fixed64_6k_v1"
STEPS = 6000
MICROBATCH = 64
REVISION_ID = "20260805T180136Z-e5545cf0"


def causal_optimizer_step(
    ...
    for start in range(0, EFFECTIVE_BATCH, microbatch):
        ...
        prediction = model(
            *model_inputs(batch, slice(start, stop)),
            teacher_actions=batch["target"][start:stop],
        )
...
def write_running_state(out_dir: Path) -> None:
    ...
    atomic_json(
        ...
        {**payload, "jobs": [{"id": "A6-O030S", "status": "running", "pid": os.getpid()}]},
    )


def train(args: argparse.Namespace) -> int:
    out_dir = Path(JOINTTRAIN_ARCH6_O030S_RESULT_ROOT)
    out_dir.mkdir(parents=True, exist_ok=True)
    batch, lineage, input_manifest = load_batch()
    torch.manual_seed(SEED)
    if args.validate_only:
        model = OperationCausalAbsolute(hidden_dim=HIDDEN_DIM, dropout=DROPOUT)
        with torch.no_grad():
            output, loss = evaluate(model, batch)
            teacher_loss = teacher_forced_mae(model, batch)
        if (
            output.shape != (64, ACTION_HORIZON, ACTION_DIM)
            or not bool(torch.isfinite(loss))
            or not math.isfinite(teacher_loss)
        ):
            raise RuntimeError("O030S CPU validation failed")
        print(json.dumps({"status": "validated", "output_shape": list(output.shape)}))
        return 0
    write_running_state(out_dir)
    if not torch.cuda.is_available():
        raise RuntimeError("A6-O030S requires one CUDA GPU")
    ...
    model = OperationCausalAbsolute(hidden_dim=HIDDEN_DIM, dropout=DROPOUT).to(device)
    ...
    pilot_path = Path(JOINTTRAIN_ARCH6_O030R_RESULT_ROOT) / "resource_pilot.json"
    ...
    if selected != {
        ...
        raise ValueError("O030R resource pilot selection drifted")
    atomic_json(
        out_dir / "resource_pilot_ref.json",
        {
            "schema_version": 1,
            "source_run_id": "a6_o030r_causal_fixed64_v2",
            "source_relative_path": "../a6_o030r_causal_fixed64_v2/resource_pilot.json",
    ...
    old_checkpoint = Path(JOINTTRAIN_ARCH6_O030R_RESULT_ROOT) / "last.pt"
    old_saved = torch.load(old_checkpoint, map_location=device, weights_only=False)
    cpu_rng_state = torch.get_rng_state()
    cuda_rng_state = torch.cuda.get_rng_state(device)
    old_model = OperationCausalAbsolute(hidden_dim=HIDDEN_DIM, dropout=DROPOUT).to(device)
    ...
    for step in range(1, STEPS + 1):
        ...
        loss, finite = causal_optimizer_step(model, optimizer, batch, MICROBATCH)
        step_times.append(time.perf_counter() - step_started)
        gradients_finite = gradients_finite and finite and math.isfinite(loss)
        if not gradients_finite:
            raise RuntimeError(f"nonfinite O030S training state at step {step}")
        if step == 1 or step % 100 == 0:
            model.eval()
            with torch.no_grad():
                prediction, evaluation_loss = evaluate(model, batch)
                teacher_evaluation_loss = teacher_forced_mae(model, batch)
            history.append(
                {
                    "step": step,
                    "train_loss": loss,
                    "eval_normalized_mae": float(evaluation_loss),
                    "teacher_forced_eval_normalized_mae": teacher_evaluation_loss,
                }
            )
            if step == 2000:
    ...
    with torch.no_grad():
        ...
        final_teacher_forced_loss = teacher_forced_mae(model, batch)
    ...
    reloaded = OperationCausalAbsolute(hidden_dim=HIDDEN_DIM, dropout=DROPOUT).to(device)
    ...
    checks = {
        ...
        "teacher_forced_eval_finite": math.isfinite(final_teacher_forced_loss),
    ...
    metrics = {
        ...
        "final_teacher_forced_normalized_mae": final_teacher_forced_loss,
        "autoregressive_over_teacher_ratio": float(final_loss) / max(final_teacher_forced_loss, 1e-12),
    ...
    summary = {
        ...
        "checks": checks,
        "decision": (
            "O030S passes the revised 6k autoregressive fixed64 gate; perform M1 revised-budget review before DYN64 routing."
            if passed
            else "O030S remains a scoped autoregressive training-fit failure at the revised 6k budget; perform M1 revised-budget review."
        ),
        "remaining_work": ["analyze O030S terminal evidence", "run CPU-only M1 revised-budget review before any DYN64 or further revision"],
        "next_run_ids": ["a6_o000d_m1_revised_fit_review_v1"],
    ...
    atomic_json(
        out_dir / "queue_state.json",
        {**summary, "jobs": [{"id": "A6-O030S", "status": summary["status"]}]},
    )
...
def main() -> int:
    ...
    try:
        ...
        summary = {
            ...
            "claim_supported": "no",
            "decision": "O030S implementation failed before valid revised-budget evidence.",
        ...
        atomic_json(
            out_dir / "queue_state.json",
            {**summary, "jobs": [{"id": "A6-O030S", "status": "failed"}]},
        )
        return 2
