#!/usr/bin/env python3
"""Corrected A6-O030R causal Transformer fixed64 memorization."""
...
from path_config import JOINTTRAIN_ARCH6_O030R_RESULT_ROOT
...
RUN_ID = "a6_o030r_causal_fixed64_v2"
...
def run_resource_pilot(
    ...
    workload_signature = {
        "family": "A6-O030R fixed64 memorization",
        "model": "PointCloudContextEncoder256+CAUSAL-ABS",
        "decoder": {
    ...
    for microbatch in MICROBATCH_CANDIDATES:
        ...
        for _ in range(PILOT_WARMUP_STEPS):
            _, finite = causal_optimizer_step(model, optimizer, batch, microbatch)
            if not finite:
                raise RuntimeError("nonfinite gradient during O030R pilot warmup")
        ...
        if not gradients_finite:
            raise RuntimeError("nonfinite gradient during O030R resource pilot")
...
def write_running_state(out_dir: Path) -> None:
    ...
    atomic_json(
        ...
        {**payload, "jobs": [{"id": "A6-O030R", "status": "running", "pid": os.getpid()}]},
    )


def train(args: argparse.Namespace) -> int:
    out_dir = Path(JOINTTRAIN_ARCH6_O030R_RESULT_ROOT)
    ...
    if args.validate_only:
        ...
        with torch.no_grad():
            ...
            teacher_output = model(
                *model_inputs(batch, slice(0, 2)), teacher_actions=batch["target"][:2]
            )
        if (
            output.shape != (2, ACTION_HORIZON, ACTION_DIM)
            or teacher_output.shape != output.shape
            or not bool(torch.isfinite(output).all())
            or not bool(torch.isfinite(teacher_output).all())
        ):
            raise RuntimeError("O030R CPU validation forward failed")
        print(json.dumps({"status": "validated", "output_shape": list(output.shape)}))
        return 0
    write_running_state(out_dir)
    if not torch.cuda.is_available():
        raise RuntimeError("A6-O030R requires one CUDA GPU")
    ...
    model_probe = OperationCausalAbsolute(hidden_dim=HIDDEN_DIM, dropout=DROPOUT)
    ...
    decoder_parameters = (
        ...
        + parameter_count(model_probe.action_embedding)
        + parameter_count(model_probe.action_head)
        + model_probe.position.numel()
        + model_probe.bos.numel()
    )
    ...
    atomic_json(
        out_dir / "run_manifest.json",
        {
            "schema_version": 1,
            "run_id": RUN_ID,
            "depends_on": ["A6-D020", "A6-D030", "A6-O000BR2", "A6-O020R terminal routing"],
            "config": config,
            "lineage": lineage,
        },
    )
    ...
    for step in range(1, STEPS + 1):
        step_started = time.perf_counter()
        loss, finite = causal_optimizer_step(model, optimizer, batch, selected_microbatch)
        step_times.append(time.perf_counter() - step_started)
        gradients_finite = gradients_finite and finite and math.isfinite(loss)
        if not gradients_finite:
            raise RuntimeError(f"nonfinite O030R training state at step {step}")
    ...
    summary = {
        ...
        "checks": checks,
        "decision": (
            "Corrected O030R fixed64 fit passes; causal arm may proceed to a separately authorized DYN64 run."
            if passed
            else "Corrected O030R is a scoped training-fit failure; all three M1 arms require attribution review."
        ),
        "remaining_work": (
            ["analyze O030R terminal evidence", "prepare causal DYN64 only if separately authorized"]
            if passed
            else ["analyze O030R terminal evidence", "audit all M1 training-fit failures before changing budget or entering DYN64"]
        ),
        "next_run_ids": (
            ["a6_o120_causal_dyn64_v1"] if passed else ["a6_o000c_m1_fit_attribution_v1"]
        ),
    ...
    atomic_json(
        out_dir / "queue_state.json",
        {**summary, "jobs": [{"id": "A6-O030R", "status": summary["status"]}]},
    )
...
def main() -> int:
    ...
    try:
        ...
        summary = {
            ...
            "claim_supported": "no",
            "decision": "O030R implementation or resource execution failed before valid fit evidence.",
        ...
        atomic_json(
            out_dir / "queue_state.json",
            {**summary, "jobs": [{"id": "A6-O030R", "status": "failed"}]},
        )
        return 2
