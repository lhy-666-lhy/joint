...
|---|---|---|---|---|---|---|---|---|
| A6-D000 | evidence/path freeze | E1-E28定向阅读 + evidence lock + `path_config.py` entries | metadata only | `historical_evidence_review.md`逐项写可信/禁止外推claim与symbols；hashes persisted；zero new full replay | MUST | reused evidence | READY | 当前唯一入口 |
| A6-D010 | exact adapter roundtrip | authoritative operation adapter | 2 fixed-gate trajectories | normalize/decode/stitch max error <=1e-6；2/2 replay parity | MUST | D000 | BLOCKED | 不是 executor requalification |
| A6-D020 | sample/normalizer freeze | 16-anchor H32/K8 index | DYN8/A5_TRAIN stats | 64 fixed chunks；mask/split/hash exact；shared normalizer | MUST | D010 | BLOCKED | no final reads |
| A6-D030 | DYN64 materialization | live 1024-point keyframes | 64 train targets | finite；2 workers；restartable；source hash exact | MUST | D010 | BLOCKED | 可与 D020 并行 |

## M0A: Split-Safe Affordance Baseline

| Run ID | Purpose | System / Variant | Split | Metrics / Gate | Priority | Depends On | Status | Notes |
|---|---|---|---|---|---|---|---|---|
| A6-A000 | affordance membership/provenance | A5 sample IDs -> Stage1 primary/aug manifest | A5_TRAIN/A5_CAL metadata | 559/102 sources；zero overlap/unjoined/duplicate/final-label read；init provenance | MUST | D000 | BLOCKED | old replay_split forbidden |
...
| Run ID | Purpose | System / Variant | Split | Metrics / Gate | Priority | Depends On | Status | Notes |
|---|---|---|---|---|---|---|---|---|
| A6-O010 | memorization | O-MLP-ABS | fixed 64 chunks | 2k steps；MAE <=1e-3；loss >=100x decrease；reload <=1e-6 | MUST | D020 | BLOCKED | ZERO affordance；simple baseline |
| A6-O020 | memorization | O-PAR-ABS | same 64 chunks | same gate | MUST | D020 | BLOCKED | ZERO affordance；ACT-like decoder |
| A6-O030 | memorization | O-CAUSAL-ABS | same 64 chunks | same gate | MUST | D020 | BLOCKED | ZERO affordance；causal decoder |
...
`A6-D000` is the only `READY` run. It must read the Architecture 1-4 references enumerated in plan E17-E28 and persist the scoped review. D000 pass unlocks CPU-only A000 and D010; A000-A030 may then proceed independently and do not block the core ZERO operation/grasp architecture/representation screens. Do not launch model training before its declared dependency, or any A5_MECH_DEV/final evaluation, Diffusion, Cartesian, DAgger, or RL before declared gates pass.
