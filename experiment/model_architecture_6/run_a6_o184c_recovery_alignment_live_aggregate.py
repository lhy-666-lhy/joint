#!/usr/bin/env python3
"""Aggregate corrected live8 recovery alignment results."""
...
from path_config import JOINTTRAIN_ARCH6_O171C_RESULT_ROOT, JOINTTRAIN_ARCH6_O184C_RESULT_ROOT
from run_a6_o010r_mlp_fixed64 import atomic_json


RUN_ID = "a6_o184c_recovery_alignment_live8_v1"
CALLS = [155, 122, 103, 650, 61, 79, 93, 86]
ARMS = ("baseline_mlp", "time_aligned", "progress_aligned", "repeat_last")
...
def main() -> int:
    out = Path(JOINTTRAIN_ARCH6_O184C_RESULT_ROOT)
    ...
    summary = {
        ...
        "scientific_scope": "corrected live8 full-horizon recovery alignment comparison",
        "metrics": metrics,
        "pairwise": pairwise,
        ...
        "checks": checks,
        "decision": "recovery alignment live screen complete; select from paired evidence"
        if passed
        else "repair O184C protocol before conclusions",
        "next_run_ids": [],
    }
    ...
    atomic_json(
        ...
        {**summary, "jobs": [{"id": "A6-O184C", "status": summary["status"]}]},
    )
