...
from path_config import JOINTTRAIN_ARCH6_O010R_RESULT_ROOT, JOINTTRAIN_ARCH6_O200F_RESULT_ROOT
...
RUN_ID = "a6_o200f_mlp_command_delta_fixed64_v1"
STEPS = 6000
MICROBATCH = 64
REVISION_ID = "20260805T185907Z-aae0560e"


def command_delta_targets(batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
    """Return command-delta targets and the normalized repeat-last base."""
    last_command_raw = batch["state"][:, -ACTION_DIM:]
    mean = batch["action_mean"].reshape(1, 1, ACTION_DIM)
    std = batch["action_std"].reshape(1, 1, ACTION_DIM)
    repeat_abs_norm = (last_command_raw.unsqueeze(1) - mean) / std
    delta_raw = batch["target_raw"] - last_command_raw.unsqueeze(1)
    delta_norm = delta_raw / std
    return delta_norm, repeat_abs_norm


def reconstruct_absolute(delta_norm: torch.Tensor, batch: dict[str, torch.Tensor]) -> torch.Tensor:
    _, repeat_abs_norm = command_delta_targets(batch)
    return repeat_abs_norm + delta_norm
...
def optimizer_step_delta(
    ...
    delta_target, _ = command_delta_targets(batch)
...
def write_running_state(out_dir: Path) -> None:
    ...
    atomic_json(
        ...
        {**payload, "jobs": [{"id": "A6-O200F", "status": "running", "pid": os.getpid()}]},
    )


def train(args: argparse.Namespace) -> int:
    out_dir = Path(JOINTTRAIN_ARCH6_O200F_RESULT_ROOT)
    ...
    config = {
        ...
        "loss": "valid-mask normalized per-dimension L1 on command delta",
        "augmentation": False,
        "seed": SEED,
        "restart": "scratch; do not resume 2k checkpoint",
        "checkpoint_rule": "save fixed 6k last checkpoint; no ABS checkpoint reproduction",
        "representation": "command_delta = target_abs - last_command_raw",
        "reconstruction": "reconstructed_abs_norm = repeat_abs_norm + predicted_delta_norm",
        "forbidden_representation": "state_start_delta = target_abs - current_actual_qpos",
    ...
    atomic_json(
        out_dir / "forbidden_feature_audit.json",
        {
            ...
            "outcome_read": False,
            "heldout_read": False,
            "source": "O000BR2 fixed_input_v2.npz only; last_command is state_history[-9:]",
        },
    )
    ...
    atomic_json(
        out_dir / "run_manifest.json",
        {
            "schema_version": 1,
            "run_id": RUN_ID,
            "depends_on": ["A6-O000D", "A6-R000", "A6-R010", "A6-R020", "A6-R030", "A6-CMDDELTA-v1.5"],
            "config": config,
            "lineage": lineage,
    ...
    for step in range(1, STEPS + 1):
        ...
        if not gradients_finite:
            raise RuntimeError(f"nonfinite O200F training state at step {step}")
    ...
    checks = {
        ...
        "zero_delta_reconstructs_repeat_last": bool(torch.max(torch.abs(reconstruct_absolute(torch.zeros_like(final_prediction), batch) - command_delta_targets(batch)[1])) <= 1e-6),
        "delta_absolute_l1_parity_le_1e_6": bool(torch.max(torch.abs((final_prediction - command_delta_targets(batch)[0]) - (reconstruct_absolute(final_prediction, batch) - batch["target"]))) <= 1e-6),
        "reconstructed_mae_better_than_repeat_last": float(final_loss) < baselines["repeat_last_command_normalized_mae"],
        "loss_decrease_ratio_finite": math.isfinite(decrease) and decrease > 0.0,
    ...
    metrics = {
        ...
        "zero_repeat_max_error": float(torch.max(torch.abs(reconstruct_absolute(torch.zeros_like(final_prediction), batch) - command_delta_targets(batch)[1]))),
        "delta_absolute_l1_parity_max_error": float(torch.max(torch.abs((final_prediction - command_delta_targets(batch)[0]) - (reconstruct_absolute(final_prediction, batch) - batch["target"])))),
    ...
    summary = {
        ...
        "checks": checks,
        "decision": (
            "O200F command-delta fixed64 fit and implementation invariants pass; use it as evidence for the clean DYN64 architecture comparison."
            if passed
            else "O200F is a scoped command-delta training-fit negative; do not classify independent architectures or the full task from this run."
        ),
        "remaining_work": [
            "build the clean TRAIN-only command-delta normalizer",
            "audit reuse of the clean DYN64 materialization",
            "compare MLP, parallel-query, and causal command-delta decoders",
        ],
        "next_run_ids": [
            "a6_d021c_command_delta_normalizer_v1",
            "a6_d030c_clean_dyn64_hash_audit_v1",
        ],
    ...
    atomic_json(
        out_dir / "queue_state.json",
        {**summary, "jobs": [{"id": "A6-O200F", "status": summary["status"]}]},
    )
...
def main() -> int:
    ...
    try:
        ...
        summary = {
            ...
            "claim_supported": "no",
            "decision": "O200F implementation failed before valid command-delta evidence.",
            "remaining_work": ["inspect failure.json and repair without changing A6-CMDDELTA-v1.5"],
            "next_run_ids": [],
            "event_id": f"{RUN_ID}_terminal",
        }
        atomic_json(out_dir / "summary.json", summary)
        ...
        atomic_json(
            out_dir / "queue_state.json",
            {**summary, "jobs": [{"id": "A6-O200F", "status": "failed"}]},
        )
        return 2
