...
- **修订日期**：2026-08-07
- **状态**：`GRASP_FACTOR_REVISION_READY / OPERATION_CANDIDATE_FROZEN / A030C_UPDATED_PRED_POSITIVE / G062C_SCOPED_NEGATIVE`
- **权威计划**：`EXPERIMENT_PLAN.md`
- **当前训练状态**：无训练进程；新revision尚未启动；首批授权为CPU/FK诊断G063与affordance lineage审计A031
- **最新抓取状态（2026-08-07）**：A030/G061 contact proposal机制为正；G062单值contact-conditioned SE3 scoped negative；转向contact-local mode、IK等价集合与handoff优化
...
|---|---|---|---|---|---|
| `A6-G063C` | M0 | contact-local orientation mode + IK-equivalent best-of-M诊断 | TRAIN构造/CAL一次评测 | MUST | READY；CPU/FK-only，首个授权抓取run |
| `A6-A031C` | M0 | initial/updated/mix060与A000 clean membership exact lineage | TRAIN/CAL audit | MUST | READY；可与G063并行 |
| `A6-G064C` | M1 | mode/residual、IK-set、sparse keypose supervision contract | TRAIN/CAL labels | MUST | BLOCKED on G063 positive oracle |
| `A6-G065C` | M1 | G062复现 vs mode+residual vs +set/FK consistency | TRAIN/CAL | MUST | BLOCKED on G064；matched seed/budget |
| `A6-G066C` | M2 | multi-IK realization + outcome-blind planner selector | CAL | MUST | BLOCKED on G065 |
| `A6-A032C` | M2 | matched one-seed PRED_INITIAL/PRED_MIX060 screen；UPDATED复用A030 | TRAIN/CAL | MUST | BLOCKED on A031 |
| `A6-G067C` | M2 | predicted-contact full proposal；相同总candidate预算 | CAL | MUST | BLOCKED on G066 |
| `A6-A033C` | M2 | UPDATED/INITIAL/MIX060/INITIAL+MIX060 consumer utility | TRAIN/CAL | MUST | BLOCKED on A032,G065 |
...
停止规则：G063无best-of-M正向曲线则不训练mode生成器；G067在相同candidate预算下无selected top-1改善则不physical；G068 strict grasp仍为0则不接O185；A033若mix/双通道无下游收益则A030 updated保持权威producer；J050/J100若牺牲contact或strict grasp则回退J000模块化系统。
