#!/usr/bin/env python3
"""Freeze the clean A6 operation sample index, fixed batch, and normalizer."""
...
RUN_ID = "a6_d020_clean_sample_normalizer_v1"
...
def main() -> int:
    out_dir = Path(JOINTTRAIN_ARCH6_D020_CLEAN_RESULT_ROOT)
    ...
    accepted = [json.loads(line) for line in accepted_path.read_text(encoding="utf-8").splitlines() if line.strip()]
    ...
    with index_tmp.open("w", encoding="utf-8") as index_handle:
        for sample_number, sample_id in enumerate(train_ids, 1):
            ...
            if sample_number % 25 == 0:
                atomic_json(out_dir / "run_state.json", {"schema_version": 1, "iteration_id": RUN_ID, "status": "running", "samples_scanned": sample_number, "samples_total": len(train_ids), "trajectories_scanned": trajectory_count})
    ...
    normalizer = {"schema_version": 1, "split": "A5_TRAIN", "source": "clean A6 operation joint_command_qpos_repaired rows", "action_dimension": 9, "row_count": command_rows, "trajectory_count": trajectory_count, "mean": mean.tolist(), "std": std.tolist()}
    ...
    excluded_train_trajectories = sum(int(row["trajectory_count"]) for row in exclusion["rows"] if row["split"] == "A5_TRAIN")
    expected_trajectories = 26605 - excluded_train_trajectories
    expected_samples = int(split["source_partition_counts"]["A5_TRAIN"]["sample_count"])
    checks = {"train_sample_count_exact": len(train_ids) == expected_samples, "train_trajectory_count_exact": trajectory_count == expected_trajectories, "excluded_sample_ids_absent": not ({row["sample_id"] for row in exclusion["rows"]} & set(train_ids)), "dyn8_target_count_exact": len({row["target"] for row in fixed_rows}) == 8, "fixed_chunk_count_exact": len(fixed_rows) == 64, "all_chunks_h32x9_finite": actions.shape == (64, 32, 9) and bool(np.isfinite(actions).all()), "valid_masks_exact": masks.shape == (64, 32) and bool(masks.any(axis=1).all()), "all_sample_splits_train": True, "terminal_mask_consumed": True, "source_hashes_persisted": True, "zero_mech_dev_or_final_reads": True}
    ...
    summary = {"schema_version": 1, "run_id": RUN_ID, "complete": True, "terminal": True, "status": "passed" if passed else "failed", "failure_class": None if passed else "data_contract_failure", "claim_supported": "yes" if passed else "no", "evidence": {"sample_index": "sample_index.jsonl", "fixed_batch": "fixed_batch.npz", "fixed_batch_manifest": "fixed_batch_manifest.json", "normalizer": "normalizer.json", "clean_split_contract_sha256": sha256_file(split_path), "clean_accepted_samples_sha256": sha256_file(accepted_path), "exclusion_manifest_sha256": sha256_file(exclusion_path)}, "counts": {"train_samples": len(train_ids), "train_trajectories": trajectory_count, "excluded_train_trajectories": excluded_train_trajectories, "operation_command_rows": command_rows, "fixed_chunks": len(fixed_rows)}, "checks": checks, "decision": "Clean D020 sample/normalizer freeze passes." if passed else "Clean D020 contract failed; do not train.", "remaining_work": ["build a clean TRAIN-only command-delta normalizer", "audit D030 reuse because its 64 targets are unchanged"] if passed else ["repair clean D020 without changing H/K or labels"], "next_run_ids": ["a6_d021c_command_delta_normalizer_v1", "a6_d030c_clean_dyn64_hash_audit_v1"] if passed else [], "event_id": f"{RUN_ID}_terminal"}
    atomic_json(out_dir / "summary.json", summary)
    atomic_json(out_dir / "run_state.json", summary)
    atomic_json(out_dir / "queue_state.json", {**summary, "jobs": [{"id": "A6-D020C", "status": summary["status"]}]})
