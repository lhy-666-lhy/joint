...
- **修订日期**：2026-08-07
- **状态**：`GRASP_REALIZABILITY_REVISION_TERMINAL_NEGATIVE / A030C_UPDATED_FROZEN / PHYSICAL_BLOCKED`
- **权威计划**：`EXPERIMENT_PLAN.md`
- **当前训练状态**：无训练或planner进程；G070C 101-group planner screen已terminal
- **最新抓取状态（2026-08-07）**：G066 S0/S1/S2均未过pose gate；G070 direct-qpose CAL L1 `0.595rad`且planner coverage仅`25.65%`；本revision停止
...
| `A6-G070C` | M2R | direct K8 relative-qpose set + current-state planner | TRAIN/CAL | MUST | SCOPED_NEGATIVE；CAL oracle L1 `0.595rad`，planner coverage `25.65%` |
