...
| A6-O000B | shared input materialization sanity | qpos/qvel/command/contact/task + XYZ/mask/ZERO | fixed64 TRAIN only | exact schema；finite；causal；URDF metadata；forbidden fields zero | MUST | O000A + revision | READY | CPU-only；blocks corrected O010R/O020R/O030R |
...
`A6-D000`, `A6-A000R`, D010-D030 and O000A are `DONE`; `A6-INPUT-v1.1` is applied. O000B is the current CPU-only input materialization sanity; corrected O010R/O020R/O030R remain blocked until it passes. A000 is blocked by two missing A5_TRAIN primary rows; A010 stays blocked. Do not launch any DYN64 model fit, physics, A5_MECH_DEV/final evaluation, Diffusion, Cartesian, DAgger, or RL before declared gates pass.
