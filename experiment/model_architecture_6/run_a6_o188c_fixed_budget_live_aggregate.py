#!/usr/bin/env python3
"""Aggregate the fixed-budget deployment-aligned operation screen."""
...
def main() -> int:
    out = Path(JOINTTRAIN_ARCH6_O188C_RESULT_ROOT)
    ...
    summary = {
        ...
        "scientific_scope": "fixed 650-call deployment-aligned operation comparison",
        "metrics": metrics,
        "pairwise": pairwise,
        "checks": checks,
        "decision": "fixed-budget screen complete; select deployable operation policy"
        if passed
        else "repair O188C before deployment conclusions",
        "next_run_ids": [],
    }
    ...
    atomic_json(
        ...
        {**summary, "jobs": [{"id": "A6-O188C", "status": summary["status"]}]},
    )
