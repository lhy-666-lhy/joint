#!/usr/bin/env python3
"""Aggregate the decisive fresh-world fixed-budget operation comparison."""
...
from path_config import JOINTTRAIN_ARCH6_O193C_RESULT_ROOT
from run_a6_o010r_mlp_fixed64 import atomic_json
from run_a6_o184c_recovery_alignment_live_aggregate import bootstrap


RUN_ID = "a6_o193c_fresh_world_fixed_budget_live8_v1"
OUTPUT_ROOT = JOINTTRAIN_ARCH6_O193C_RESULT_ROOT
SCIENTIFIC_SCOPE = "fresh-world fixed-budget operation comparison"
DECISION = "fresh-world fixed-budget screen complete; analyze paired model evidence"
MAX_CALLS = 650
EXECUTE_PREFIX = 8
...
def main() -> int:
    ...
    metrics = {arm: arm_metrics(rows) for arm, rows in by_arm.items()}
    pairwise = {}
    ...
    checks = {
        ...
        "correct_start_command_lineage": all(
            ...
            and max(abs(value) for value in row["operation_start_command_finger"]) <= 1e-7
    ...
    summary = {
        ...
        "pairwise": pairwise,
        "checks": checks,
        "decision": DECISION if passed else f"repair {RUN_ID} before conclusions",
        "next_run_ids": [],
    }
    ...
    atomic_json(
        ...
        {**summary, "jobs": [{"id": "A6-O193C", "status": summary["status"]}]},
    )
