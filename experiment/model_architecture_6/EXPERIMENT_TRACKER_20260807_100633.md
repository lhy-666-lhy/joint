...
- **修订日期**：2026-08-07
- **状态**：`G066C_SELECTOR_AND_REALIZABILITY_NEGATIVE / A032C_NO_REPLACEMENT / G070C_PLANNER_RUNNING / PHYSICAL_BLOCKED`
- **权威计划**：`EXPERIMENT_PLAN.md`
- **当前训练状态**：G070C direct-qpose formal已完成；101-group planner screen在tmux `a6_g070_planner`运行，完成标志为`results/a6_g070c_direct_qpose_set_fit_v1/planner/full/summary.json`
- **最新抓取状态（2026-08-07）**：G066 S0/S1/S2均未过pose gate；full oracle只有34.82%合法IK、7.59% planner path，瓶颈定位到SE(3)候选可实现性；G070测试直接关节空间候选
...
| `A6-G066C` | M2 | S0 mode-logit vs S1 calibrated likelihood vs S2 IK/FK/planner selector | TRAIN calibration/CAL一次评测 | MUST | SCOPED_NEGATIVE；S1仍退化，S2更差；G067未解锁 |
| `A6-A032C` | M2 | matched PRED_INITIAL/PRED_MIX060；UPDATED复用A030 | TRAIN/CAL | MUST | DONE / NO_REPLACEMENT；initial trade-off，mix060单通道负向 |
| `A6-G067C` | M2 | predicted-contact full proposal；相同总candidate预算 | CAL | MUST | BLOCKED / STOPPED；G066 selected gate失败 |
| `A6-A033C` | M2 | UPDATED/INITIAL/MIX060/[INITIAL,MIX060] consumer utility | TRAIN/CAL | MUST | BLOCKED / STOPPED；无winning G066 consumer且A032无producer winner |
| `A6-G070C` | M2R | direct K8 relative-qpose set + current-state planner | TRAIN/CAL | MUST | RUNNING planner；6000-step fit通过但CAL oracle qpose L1 `0.595rad` |
