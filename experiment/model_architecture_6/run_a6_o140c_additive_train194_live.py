#!/usr/bin/env python3
"""Run O126 live8 with additive TRAIN194 MLP/PAR checkpoints."""
...
def main() -> int:
    ...
    live.JOINTTRAIN_ARCH6_O126C_RESULT_ROOT = JOINTTRAIN_ARCH6_O140C_RESULT_ROOT
    code = live.main()
    out = Path(JOINTTRAIN_ARCH6_O140C_RESULT_ROOT) / (
        f"probe_calls_{args.max_calls}_target_{args.target_index}"
    )
    summary = json.load(open(out / "summary.json"))
    summary["run_id"] = "a6_o140c_additive_train194_live8_v1"
    summary["scientific_scope"] = (
        "A5_CAL source-horizon live closed loop for D044C additive TRAIN194"
    )
