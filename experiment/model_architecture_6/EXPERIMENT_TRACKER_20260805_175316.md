...
| A6-O000C | M1 all-branch fit attribution | reload O010R/O020R/O030R checkpoints | same fixed64 TRAIN only | same-semantic MAE；causal TF-vs-AR；per-horizon/repeat structure；embedding separability；zero training/replay | MUST | all corrected M1 terminal | RUNNING | CPU-only单进程；决定planning revision；DYN64仍锁定 |
...
All corrected M1 arms are valid scoped training-fit failures. O030R autoregressive MAE is 0.117449 with 7.29x decrease and 1032% worse than repeat-last; its teacher-forced train loss 0.016597 is not yet a same-semantic evaluation. A6-O000C is the sole READY CPU-only attribution audit. No DYN64, physics, heldout evaluation, budget/loss/input change, or conditional complexity is allowed before O000C evidence and a planning revision if required. A000/A010 remain blocked independently.
