...
- **修订日期**：2026-08-06
- **状态**：`OPERATION_CANDIDATE_FROZEN / G000C_DONE / G005C_READY`
- **权威计划**：`EXPERIMENT_PLAN.md`
- **当前训练状态**：无训练、无残留 Architecture 6 实验进程；O185 seed1 为 provisional operation consumer，O127 为 fallback；G000C v2 terminal pass，`A6-G005C` 已 READY
...
|---|---|---|---|---|
| `A6-G000C` | qpath/qpose label-set contract | qpath、open terminal qpose、closed operation state 分离 | O199C operation decision | DONE |
| `A6-G005C` | interface oracle pair | GT-TRAJ vs GT-QPOSE+online planner；exact-paired | G000C | READY |
