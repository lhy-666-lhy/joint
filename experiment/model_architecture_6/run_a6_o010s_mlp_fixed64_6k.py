...
from path_config import JOINTTRAIN_ARCH6_O010R_RESULT_ROOT, JOINTTRAIN_ARCH6_O010S_RESULT_ROOT
...
RUN_ID = "a6_o010s_mlp_fixed64_6k_v1"
STEPS = 6000
MICROBATCH = 64
...
def write_running_state(out_dir: Path) -> None:
    ...
    atomic_json(
        ...
        {**payload, "jobs": [{"id": "A6-O010S", "status": "running", "pid": os.getpid()}]},
    )


def train(args: argparse.Namespace) -> int:
    out_dir = Path(JOINTTRAIN_ARCH6_O010S_RESULT_ROOT)
    ...
    old_checkpoint = Path(JOINTTRAIN_ARCH6_O010R_RESULT_ROOT) / "last.pt"
    old_saved = torch.load(old_checkpoint, map_location=device, weights_only=False)
    cpu_rng_state = torch.get_rng_state()
    cuda_rng_state = torch.cuda.get_rng_state(device)
    old_model = OperationMLPAbsolute(hidden_dim=HIDDEN_DIM, dropout=DROPOUT).to(device)
    ...
    for step in range(1, STEPS + 1):
        ...
        if not gradients_finite:
            raise RuntimeError(f"nonfinite O010S training state at step {step}")
    ...
    summary = {
        ...
        "checks": checks,
        "decision": (
            "O010S passes the revised 6k fixed64 gate; continue independent O020S before DYN64 routing."
            if passed
            else "O010S remains a scoped training-fit failure at the revised 6k budget; continue independent O020S."
        ),
        "remaining_work": ["analyze O010S terminal evidence", "route independent O020S under A6-FIT-v1.2"],
        "next_run_ids": ["a6_o020s_par_fixed64_6k_v1"],
    ...
    atomic_json(
        out_dir / "queue_state.json",
        {**summary, "jobs": [{"id": "A6-O010S", "status": summary["status"]}]},
    )
...
def main() -> int:
    ...
    try:
        ...
        summary = {
            ...
            "claim_supported": "no",
            "decision": "O010S implementation failed before valid revised-budget evidence.",
        ...
        atomic_json(
            out_dir / "queue_state.json",
            {**summary, "jobs": [{"id": "A6-O010S", "status": "failed"}]},
        )
        return 2
